<div class="form-group">
    ...
    <div id="selectbox-config-{{$classAttribute->id}}" style="background: #f4f4f4;padding: 15px;border-radius: 3px">
        ...
        <div style="width: 300px; display:inline-block;">Spaltenname</div>
        <div style="width: 120px; display:inline-block;">Breite</div>
        <br />
        <div class="rows">
            @if($classAttribute->configuration)
                @foreach($classAttribute->configuration as $key => $row)
                    <div style="padding-bottom: 3px">
                        ...
                        <input type="text" name="configuration[{{$classAttribute->id}}][{{$key}}][value]" value="{{$row["value"]}}" class="form-control" style="display: inline-block; width:300px" />
                        <input type="text" name="configuration[{{$classAttribute->id}}][{{$key}}][width]" value="{{$row["width"]}}" class="form-control" style="display: inline-block; width:300px" />
        ...
        <br />
        <a href="javascript:;" class="add-row btn btn-primary">neue Spalte</a>
    </div>
</div>
<script>
    ;(function(){
        ...
        container.find(".add-row").click(function(){
            ...
            container.find(".rows").append(
                '<div style="padding-bottom: 3px">' +
                    ...
                    '<input type="text" name="configuration[{{$classAttribute->id}}][' + i + '][value]" value="" class="form-control" style="display: inline-block; width:300px" /> ' +
                    '<input type="text" name="configuration[{{$classAttribute->id}}][' + i + '][width]" value="" class="form-control" style="display: inline-block; width:100px" /> ' +
                    '<a href="javascript:;" class="cancel remove-row">&times;</a>' +
                '</div>'
            );
        });
