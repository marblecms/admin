        ...
        <div class="main-box-body clearfix">
            <form action="{{ url("admin/nodeclass/save/" . $nodeClass->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="namedIdentifier" value="{{$nodeClass->namedIdentifier}}" class="form-control"/>
                </div>   
                ...
                <div class="form-group">
                    ...
                    <select name="allowChildren" class="form-control">
                        <option value="1" {{ $nodeClass->allowChildren == 1 ? 'selected="selected"' : '' }}>ja</option>
                        <option value="0" {{ $nodeClass->allowChildren == 0 ? 'selected="selected"' : '' }}>nein</option>
                    </select>
                </div>
                
                <div class="form-group">
                    <label>Gruppen als Tabs</label>
                    <select name="tabs" class="form-control">
                        <option value="1" {{ $nodeClass->tabs == 1 ? 'selected="selected"' : '' }}>ja</option>
                        <option value="0" {{ $nodeClass->tabs == 0 ? 'selected="selected"' : '' }}>nein</option>
                    </select>
                </div>
            
                <div class="form-group">
                    <label>Kinder auflisten</label>
                    <select name="listChildren" class="form-control">
                        <option value="1" {{ $nodeClass->listChildren == 1 ? 'selected="selected"' : '' }}>ja</option>
                        <option value="0" {{ $nodeClass->listChildren == 0 ? 'selected="selected"' : '' }}>nein</option>
                    </select>
                </div>
            
                <div class="form-group">
                    <label>In Baum anzeigen</label>
                    <select name="showInTree" class="form-control">
                        <option value="1" {{ $nodeClass->showInTree == 1 ? 'selected="selected"' : '' }}>ja</option>
                        <option value="0" {{ $nodeClass->showInTree == 0 ? 'selected="selected"' : '' }}>nein</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="groupId" class="form-control">
                        @foreach($nodeClassGroups as $nodeClassGroup)
                            <option value="{{$nodeClassGroup->id}}" {{ $nodeClassGroup->id == $nodeClass->groupId ? 'selected="selected"' : '' }}>{{$nodeClassGroup->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select multiple name="allowedChildClasses[]" class="form-control" size="10">
                        <option value="all" {{ (in_array("all",$nodeClass->allowedChildClasses) ? 'selected="selected"' : '')}} >- Alle -</option>
                        @foreach($groupedNodeClasses as $nodeClasses)
                            ...
                            @foreach($nodeClasses->items as $item)
                                <option value="{{$item->id}}" {{ (in_array($item->id,$nodeClass->allowedChildClasses) ? 'selected="selected"' : '') }}>&nbsp; &nbsp; &nbsp; {{$item->name}}</option>
                            @endforeach
                        @endforeach
                    </select>
                </div>
