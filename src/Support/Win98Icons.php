...
class Win98Icons
{
    ...
    public static function url(string $famiconName, string $theme = 'xp'): string
    {
        if ($theme === '98') {
            $map = static::map();
            if (isset($map[$famiconName])) {
                return asset('vendor/marble/assets/images/win98icons/' . $map[$famiconName]);
            }
        }

        return asset('vendor/marble/assets/images/famicons/' . $famiconName . '.svg');
    }
}
